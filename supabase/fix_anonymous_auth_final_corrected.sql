...
-- ALTER TABLE文とUPDATE文は別々に記述する必要があります
UPDATE auth.users 
...
-- アプリで使用するデバイスIDも受け入れるポリシー
CREATE POLICY "Device IDs can be used as user IDs" ON groups
  FOR ALL USING (true);

CREATE POLICY "Device IDs can be used as member IDs" ON group_members
  FOR ALL USING (true);
